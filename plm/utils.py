...
class MorphParseDataset:
    ...
    def load(self):
        ...
        train_path = os.path.join(
            self.path, "TRAIN", f"{self.language}_TRAIN{self.file_suffix}.tsv"
        )
        test_path = os.path.join(
            self.path, "TEST", f"{self.language}_TEST{self.file_suffix}.tsv"
        )
...
class GenUtils:
    def align_seqs(pred: list, actual: list, pad="<?UNK?>"):
        ...
        for indices in GenUtils._gen_indices(
            len(longer) - len(shorter), len(longer) - 1
        ):
            # Just copy the shorter one as it's what we will insert into
            pred_copy, actual_copy = (
                (pred, GenUtils._copy_and_insert(actual, indices, pad))
                if len(pred) > len(actual)
                else (GenUtils._copy_and_insert(pred, indices, pad), actual)
            )
