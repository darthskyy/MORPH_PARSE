...
for i, language in enumerate(languages):
    lines = []
    with open(f"data/TRAIN/{language}_TRAIN.tsv") as f:
        lines_ = f.readlines()
        lines.extend(lines_)

    with open(f"data/TEST/{language}_TEST.tsv") as f:
        lines_ = f.readlines()
        lines.extend(lines_)
    ...
    axs[i, 1].pie([word_counts[language] - word_morpheme_matches[language], word_morpheme_matches[language]],
                  labels=["Non-Matches", "Surface-Canonical Matches"], autopct='%1.1f%%')
...
for language in languages:
    ...
    print(
        f"Percentage of words that match the morphemes: {word_morpheme_matches[language] / word_counts[language] * 100:.2f}%")
